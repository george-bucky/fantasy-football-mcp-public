...
try:
    ...
    try:
        query = YahooFantasySportsQuery(
            ...
            game_code="nfl",
            game_id=None,  # Resolve Yahoo's current NFL season dynamically
        ...
        try:
            ...
            # Try to get leagues
            user_leagues = query.get_user_leagues_by_game_key("nfl")
            if user_leagues:
            ...
            access_token = getattr(query.oauth, "access_token", None)
            refresh_token = getattr(query.oauth, "refresh_token", None)
            guid = getattr(query.oauth, "guid", None)
            if access_token and refresh_token:
                update_env_file_with_tokens(
                    access_token, refresh_token, ENV_FILE_PATH, guid=guid
                )
                update_mcp_configs(access_token, refresh_token, guid=guid)
                preflight_fantasy_access(access_token)
                print("\n✅ OAuth credentials saved to the local .env file")
                print("   The MCP server can now use these credentials!")
            else:
                print("⚠️  Authentication completed but OAuth tokens were unavailable")
...
print("1. OAuth credentials are saved to the local .env file")
print("2. The MCP server will load them automatically")
